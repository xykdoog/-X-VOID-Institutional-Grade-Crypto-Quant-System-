...
# ==========================================
# 🔥 Telegram 洪水防御系统 (Rate Limiting)
# ==========================================
# Telegram 限制：每秒最多 30 条消息，单个聊天每分钟最多 20 条
MAX_MSG_PER_MINUTE = 20 
_msg_history = deque(maxlen=200)  # 🔥 审计修复: 限制消息历史上限，防止无限增长
_msg_lock = threading.Lock()

# 🔥 洪水防御的限流检查函数（供下方 send_tg_msg 使用）
def _check_rate_limit():
    """检查是否触发限流，返回 True 表示允许发送"""
    with _msg_lock:
        current_time = time.time()
        while _msg_history and _msg_history[0] < current_time - 60:
            _msg_history.popleft()
        if len(_msg_history) >= MAX_MSG_PER_MINUTE:
            logger.warning(f"⚠️ [洪水防御] 触发报警限流，当前 60s 内已发送 {len(_msg_history)} 条。")
            return False
        _msg_history.append(current_time)
        return True
...
# ==========================================
# 🔥 环境感知风险检查（SANDBOX/LIVE 隔离）
# ==========================================

def check_risk_with_environment_isolation(client=None, max_drawdown_pct=0.20):
    """
    环境感知风险检查函数（替代原 check_circuit_breaker 的风险检查部分）
    
    核心逻辑：
    1. 环境识别：首先读取 config.RUNNING_MODE
    2. 动态余额获取：
       - 如果是 SANDBOX，必须使用本地 sim_balance 或 sandbox_ledger.json 中的虚拟余额
       - 如果是 LIVE，才允许调用币安 API 获取实盘余额
    3. 计算隔离：严禁在沙盒模式下引用任何实盘 API 返回的 Balance 数据进行风险率计算
    
    Args:
        client: Binance 客户端（仅在 LIVE 模式下使用）
        max_drawdown_pct: 最大回撤百分比阈值（默认 20%）
    
    Returns:
        dict: {
            'allowed': bool,           # 是否允许继续交易
            'current_balance': float,  # 当前余额
            'benchmark': float,        # 基准本金
            'drawdown_pct': float,     # 当前回撤百分比
            'mode': str,               # 运行模式 (SANDBOX/LIVE)
            'balance_source': str,     # 余额来源 (sim_balance/sandbox_ledger/binance_api)
            'message': str             # 检查结果消息
        }
    """
    ...
    try:
        ...
        with state_lock:
            ...
            benchmark = SYSTEM_CONFIG.get("BENCHMARK_CASH", 10000.0)
            peak_equity = SYSTEM_CONFIG.get("PEAK_EQUITY", benchmark)
        ...
        if running_mode == "SANDBOX":
            # 🏖️ SANDBOX 模式：严格使用本地虚拟余额
            logger.debug("🏖️ [环境感知] SANDBOX 模式检测到，使用本地虚拟余额")
            
            # 优先级1：使用 SIM_CURRENT_BALANCE（实时更新的模拟余额）
            with state_lock:
                sim_balance = SYSTEM_CONFIG.get("SIM_CURRENT_BALANCE", None)
            
            if sim_balance is not None and sim_balance > 0:
                current_balance = sim_balance
                balance_source = "sim_balance"
                logger.debug(f"   ✅ 使用 SIM_CURRENT_BALANCE: ${current_balance:.2f}")
            else:
                # 优先级2：从 Redis 或 sandbox_ledger.json 读取最新余额
                try:
                    import json
                    import os
                    
                    # 🔥 审计修复 6.2: 优先从 Redis 读取沙盒余额
                    _ledger_loaded = False
                    try:
                        from redis_manager import redis_db
                        if redis_db.enabled:
                            redis_data = redis_db.load_hash("wjbot:sandbox:ledger")
                            if redis_data and 'balance' in redis_data:
                                current_balance = float(redis_data['balance'])
                                balance_source = "sandbox_ledger_redis"
                                logger.debug(f"   ✅ 从 Redis 读取沙盒余额: ${current_balance:.2f}")
                                _ledger_loaded = True
                    except Exception:
                        pass
                    ...
                    if not _ledger_loaded:
                        ...
                        if os.path.exists(sandbox_ledger_file):
                            with open(sandbox_ledger_file, 'r', encoding='utf-8') as f:
                                ledger_data = json.load(f)
                                current_balance = ledger_data.get("balance", ledger_data.get("current_balance", benchmark))
                                balance_source = "sandbox_ledger"
                                logger.debug(f"   ✅ 从 sandbox_ledger.json 读取余额: ${current_balance:.2f}")
                        else:
                            current_balance = benchmark
                            balance_source = "benchmark_fallback"
                            logger.warning(f"   ⚠️ sandbox_ledger.json 不存在，使用 BENCHMARK_CASH: ${current_balance:.2f}")
                except Exception as ledger_e:
                    logger.error(f"   ❌ 读取沙盒余额失败: {ledger_e}")
                    current_balance = benchmark
                    balance_source = "benchmark_fallback"
            
            # 🔒 严格隔离：SANDBOX 模式下禁止调用实盘 API
            if client is not None:
                logger.warning("⚠️ [环境隔离] SANDBOX 模式下检测到 client 参数，已忽略（不调用实盘 API）")
        
        elif running_mode == "LIVE":
            # 🔴 LIVE 模式：调用币安 API 获取实盘余额
            logger.debug("🔴 [环境感知] LIVE 模式检测到，调用币安 API 获取实盘余额")
            
            if client is None:
                logger.error("❌ [环境隔离] LIVE 模式下 client 为 None，无法获取实盘余额")
                return {
                    'allowed': False,
                    'current_balance': 0.0,
                    'benchmark': benchmark,
                    'drawdown_pct': 0.0,
                    'mode': running_mode,
                    'balance_source': 'error_no_client',
                    'message': 'LIVE 模式下缺少 Binance 客户端，无法获取实盘余额'
                }
            
            try:
                # 调用币安 API 获取实盘余额
                acc_info = client.futures_account()
                current_balance = float(acc_info.get('totalMarginBalance', 0))
                balance_source = "binance_api"
                logger.debug(f"   ✅ 从币安 API 获取实盘余额: ${current_balance:.2f}")
                
                if current_balance <= 0:
                    logger.error(f"❌ [环境隔离] 币安 API 返回余额异常: ${current_balance:.2f}")
                    return {
                        'allowed': False,
                        ...
                        'message': f'币安 API 返回余额异常: ${current_balance:.2f}'
                    }
            
            except Exception as api_e:
                logger.error(f"❌ [环境隔离] 调用币安 API 失败: {api_e}")
                return {
                    'allowed': False,
                    'current_balance': 0.0,
                    'benchmark': benchmark,
                    'drawdown_pct': 0.0,
                    'mode': running_mode,
                    'balance_source': 'api_error',
                    'message': f'调用币安 API 失败: {str(api_e)[:100]}'
                }
        
        else:
            # 未知模式
            logger.error(f"❌ [环境隔离] 未知的 RUNNING_MODE: {running_mode}")
            return {
                'allowed': False,
                'current_balance': 0.0,
                'benchmark': benchmark,
                'drawdown_pct': 0.0,
                'mode': running_mode,
                'balance_source': 'unknown_mode',
                'message': f'未知的运行模式: {running_mode}'
            }
        
        # ====== Step 3: 计算回撤率（环境隔离完成后的安全计算） ======
        if peak_equity > 0:
            drawdown_pct = ((peak_equity - current_balance) / peak_equity) * 100
        else:
            drawdown_pct = 0.0
        ...
        if drawdown_pct > max_drawdown_pct * 100:
            # 回撤超限，拒绝交易
            message = (
                f"回撤超限: {drawdown_pct:.2f}% > {max_drawdown_pct*100:.2f}% "
                f"(当前余额: ${current_balance:.2f}, 峰值: ${peak_equity:.2f})"
            )
            logger.warning(f"🚨 [风险检查] {message}")
        ...
        # 风险检查通过
        message = (
            f"风险检查通过: 回撤 {drawdown_pct:.2f}% < {max_drawdown_pct*100:.2f}% "
            f"(当前余额: ${current_balance:.2f}, 来源: {balance_source})"
        )
        logger.debug(f"✅ [风险检查] {message}")
        ...
        logger.error(f"❌ [风险检查] 环境感知风险检查异常: {e}", exc_info=True)
        return {
            'allowed': False,
            'current_balance': 0.0,
            'benchmark': 0.0,
            'drawdown_pct': 0.0,
            'mode': 'error',
            'balance_source': 'exception',
            'message': f'风险检查异常: {str(e)[:100]}'
        }
                ...
                try:
                    ...
                    try:
                        if hasattr(result, 'headers'):
                            from api_weight_monitor import update_api_weight
                            update_api_weight(result.headers)
...
def get_current_price(client, symbol):
    """
    获取指定币种的当前价格（机构级优化：WebSocket优先 + 1.5秒全局缓存与批量查询）
    
    🔥 优化：优先从 WebSocket ticker_cache 读取实时价格（秒级精度）
    回退到 REST API 缓存（1.5秒精度）
    
    价格来源优先级：
    1. WebSocket ticker_cache（实时推送，0延迟）
    2. REST API 全局缓存（1.5秒TTL）
    3. REST API 批量查询（缓存失效时）
    """
    if client is None:
        logger.info(f"模拟获取 {symbol} 价格")
        return 50000.0 if symbol == "BTCUSDT" else 3000.0 if symbol == "ETHUSDT" else 100.0
    
    # 🔥 优先级1: 尝试从 WebSocket ticker_cache 读取（实时价格）
    try:
        from websocket_manager import get_cached_ticker
        ticker_data = get_cached_ticker(symbol)
        
        if ticker_data and 'price' in ticker_data:
            # 检查 WebSocket 数据新鲜度（5秒内有效）
            ws_timestamp = ticker_data.get('timestamp')
            if ws_timestamp:
                age = (datetime.now() - ws_timestamp).total_seconds()
                if age < 5.0:
                    price = ticker_data['price']
                    logger.debug(f"✅ [{symbol}] 使用 WebSocket 实时价格: {price:.4f} (延迟 {age:.2f}s)")
                    return price
                else:
                    logger.debug(f"⚠️ [{symbol}] WebSocket 价格过期 ({age:.1f}s)，回退到 REST API")
    except ImportError:
        logger.debug("⚠️ websocket_manager 模块未加载，跳过 WebSocket 价格查询")
    except Exception as ws_e:
        logger.debug(f"⚠️ WebSocket 价格查询异常: {ws_e}")
    
    # 🔥 优先级2: 尝试从 REST API 缓存读取（1.5秒有效期）
    with _price_cache_lock:
        if time.time() - _global_price_cache['timestamp'] < 1.5:
            if symbol in _global_price_cache['prices']:
                price = _global_price_cache['prices'][symbol]
                logger.debug(f"✅ [{symbol}] 使用 REST API 缓存价格: {price:.4f}")
                return price
    
    # 🔥 优先级3: 缓存失效，触发全量 API 请求
    max_retries = NETWORK_CONFIG["MAX_RETRIES"]
    retry_delay = NETWORK_CONFIG["RETRY_DELAY"]
    ...
    for attempt in range(max_retries):
        try:
            ...
            price = _global_price_cache['prices'].get(symbol, 0.0)
            logger.debug(f"✅ [{symbol}] 使用 REST API 批量查询价格: {price:.4f}")
            return price
...
def get_kline_chart_buffer(df, symbol="", num_candles=80, mav=(7, 25, 99)):
    ...
    图片以 PNG bytes 返回，可直接喂给 Telegram send_photo 或 Claude Vision (future)。
    ...
    try:
        ...
        # ── 0. 列名兼容：Binance 返回小写，mplfinance 要求大写 ──
...
# ==========================================
# 🔍 Intelligence Hub - 智能情报中心桥接
# ==========================================

def get_intelligence_hub_instance(proxy_url=None):
    """
    获取 IntelligenceHub 全局单例（桥接函数）
    
    Args:
        proxy_url: 代理服务器地址（可选）
    
    Returns:
        IntelligenceHub: 情报中心实例
    """
    try:
        from intelligence_hub import get_intelligence_hub
        return get_intelligence_hub(proxy_url=proxy_url)
    except ImportError:
        logger.warning("⚠️ intelligence_hub 模块未找到，情报中心功能不可用")
        return None
    except Exception as e:
        logger.error(f"❌ 获取 IntelligenceHub 实例失败: {e}")
        return None


def trigger_intelligence_search(indicator_cache, proxy_url=None):
    """
    触发智能情报搜索（便捷函数）
    
    Args:
        indicator_cache: 指标缓存字典，包含 volatility, price_deviation 等
        proxy_url: 代理服务器地址（可选）
    
    Returns:
        str: 情报简报文本（如果触发），否则返回空字符串
    """
    try:
        hub = get_intelligence_hub_instance(proxy_url=proxy_url)
        if hub is None:
            return ""
        
        return hub.inject_to_ai_context(indicator_cache)
    except Exception as e:
        logger.error(f"❌ 触发智能情报搜索失败: {e}")
        return ""


# ==========================================
# 🔥 LLM 串行队列 (Producer-Consumer 模式)
# ==========================================
# 所有 AI 调用统一入队，由单线程 worker 串行消费，
# 彻底消除 asyncio.new_event_loop() 并发冲突和事件循环泄漏。

_llm_queue = queue.Queue()
_llm_worker_thread = None
_llm_loop = None  # worker 独占的事件循环


def _llm_worker_main():
    """LLM 串行消费者线程：独占一个 asyncio 事件循环，逐条处理请求"""
    global _llm_loop
    _llm_loop = asyncio.new_event_loop()
    asyncio.set_event_loop(_llm_loop)
    logger.info("🤖 LLM Worker 已启动 (单线程串行消费)")

    while True:
        task = None
        try:
            task = _llm_queue.get()  # 阻塞等待
            if task is None:
                break  # 毒丸退出
            ...
            try:
                result = _llm_loop.run_until_complete(coro_func(*args, **kwargs))
                future['result'] = result
                future['error'] = None
            except Exception as e:
                future['result'] = None
                future['error'] = e
                logger.error(f"❌ LLM Worker 执行异常: {e}")
            finally:
                future['done'].set()

        except Exception as e:
            logger.error(f"❌ LLM Worker 循环异常: {e}")
            if task and 'future' in task:
                task['future']['error'] = e
                task['future']['done'].set()


def start_llm_worker():
    """启动 LLM 串行消费者线程（幂等，重复调用安全）"""
    global _llm_worker_thread
    if _llm_worker_thread is not None and _llm_worker_thread.is_alive():
        return
...
def submit_llm_task(coro_func, *args, timeout=120, **kwargs):
    """
    提交一个异步 LLM 协程到串行队列，阻塞等待结果返回。

    用法（替代原来的 new_event_loop + run_until_complete）：
        # 旧:
        #   _loop = asyncio.new_event_loop()
        #   result = _loop.run_until_complete(commander.ask_commander(prompt))
        #   _loop.close()
        # 新:
        result = submit_llm_task(commander.ask_commander, prompt)

    Args:
        coro_func: 异步协程函数（不是已调用的协程对象）
        *args: 传给 coro_func 的位置参数
        timeout: 最大等待秒数，默认 120s
    ...
    Raises:
        TimeoutError: 超时
        Exception: 协程内部异常会被重新抛出
    """
    start_llm_worker()  # 确保 worker 已启动
    ...
    if not future['done'].wait(timeout=timeout):
        raise TimeoutError(f"LLM 请求超时 ({timeout}s)")
...
def submit_llm_task_async(bot_client, chat_id, coro_func, *args, **kwargs):
    """
    异步提交 LLM 任务到队列，立即返回（非阻塞）
    ...
    用法（替代原来的阻塞调用）：
        ...
        # 新:
        submit_llm_task_async(bot, chat_id, commander.ask_commander, prompt)
        bot.send_message(chat_id, "🧠 指挥官正在深度推演中，请稍候...")
    
    Args:
        bot_client: Telegram Bot 实例（用于推送结果）
        chat_id: 目标聊天ID
        coro_func: 异步协程函数
        *args: 传给 coro_func 的位置参数
        **kwargs: 传给 coro_func 的关键字参数
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                # 异步推送模式：直接发送结果到 TG
                if async_mode:
                    bot_client = future.get('bot_client')
                    chat_id = future.get('chat_id')
                    if bot_client and chat_id:
                        try:
                            # 清理命令标记
                            if "###COMMAND###" in result:
                                start = result.find("###COMMAND###")
                                end = result.rfind("###COMMAND###")
                                if start < end:
                                    result = result[:start] + result[end + len("###COMMAND###"):]
                            
                            # 截断过长消息
                            if len(result) > 4000:
                                result = result[:4000] + "\n\n...(内容过长已截断)"
                            
                            bot_client.send_message(chat_id, result, parse_mode="HTML")
                        except Exception as send_err:
                            logger.error(f"❌ LLM Worker 推送结果失败: {send_err}")
...
# ==========================================
# 🔥 定期内存清理 (审计P2修复: 防止长期运行OOM)
# ==========================================
_memory_cleanup_thread = None

def periodic_memory_cleanup():
    """定期内存清理守护线程：每小时清理过期缓存 + 强制GC"""
    import gc
    while True:
        try:
            ...
            # 清理过期价格缓存
            with _price_cache_lock:
                if time.time() - _global_price_cache['timestamp'] > 300:
                    _global_price_cache['prices'].clear()
                    _global_price_cache['timestamp'] = 0
            
            # 强制垃圾回收
            collected = gc.collect()
            logger.info(f"🧹 定期内存清理完成 (GC回收 {collected} 个对象)")
            
        except Exception as e:
            logger.error(f"❌ 定期内存清理异常: {e}")

def start_memory_cleanup_worker():
    """启动定期内存清理守护线程"""
    global _memory_cleanup_thread
    if _memory_cleanup_thread is not None and _memory_cleanup_thread.is_alive():
        return
    _memory_cleanup_thread = threading.Thread(
        target=periodic_memory_cleanup,
        name="MemoryCleanupWorker",
        daemon=True
    )
    _memory_cleanup_thread.start()
    logger.info("🧹 定期内存清理线程已启动 (每小时执行)")

# 自动启动内存清理
start_memory_cleanup_worker()
