...
class AIModelFallback:
    """AI模型降级管理器（🔥 纯Claude版本：已移除Gemini）"""
    
    def __init__(self, claude_commander=None):
        self.claude = claude_commander
        self.fallback_count = 0
        self.last_fallback_time = None
    
    def ask_with_fallback(self, prompt, primary='claude'):
        """
        带降级的AI调用（纯Claude，失败时返回安全响应）
        
        Args:
            prompt: 提示词
            primary: 主模型（仅支持 'claude'）
        ...
        try:
            if self.claude:
                ...
                raise Exception("Claude Commander未初始化")
        
        except Exception as e:
            logger.error(f"❌ Claude调用失败: {e}，返回安全降级响应")
...
if __name__ == "__main__":
    ...
    fallback_mgr = AIModelFallback(claude)
    
    result = fallback_mgr.ask_with_fallback("分析市场")
    print(f"场景1 - Claude成功: {result}")
    
    # 场景2: Claude失败，返回安全降级响应
    claude_fail = MockCommander("Claude", should_fail=True)
    fallback_mgr2 = AIModelFallback(claude_fail)
    
    result = fallback_mgr2.ask_with_fallback("分析市场")
    print(f"场景2 - Claude失败，安全降级: {result}")
    
    # 场景3: Claude未初始化
    fallback_mgr3 = AIModelFallback(None)
    
    result = fallback_mgr3.ask_with_fallback("分析市场")
    print(f"场景3 - Claude未初始化，安全降级: {result}")
